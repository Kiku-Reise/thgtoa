...
-   [Appendix E: Using System Rescue to securely wipe an SSD drive.]
...
-   [Appendix L: Considerations for using external SSD drives]
    ...
    -   [Linux:][10]
        -   [Trim Support:][11]
        -   [ATA/NVMe Operations (Secure Erase/Sanitize):][12]
    -   [MacOS:][13]
        -   [Trim Support:][14]
        -   [ATA/NVMe Operations (Secure Erase/Sanitize):][15]
-   [Appendix M: Creating a mat2-web guest VM for removing metadata from files]
-   [Appendix N: BIOS/UEFI options to wipe disks in various Brands]
-   [Acknowledgements:]

(Or "How I learned to start worrying and love privacy")

Version 0.5.7 (draft), January 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
Donations are welcome at <https://anonymousplanet.github.io/thgtoa/donations.html> or within the donations.md file of the repository **(please do verify the checksum and gpg signature of the file for authenticity)**.
...
-   Using an anonymous VPN service[^20] (paid by cash).
...
In addition to limited browser support, only Web Services and CDNs[^34] behind Cloudflare CDN support eSNI at this stage (and will support ECH in the future[^35]).
...
Here is an example of EXIF data that could be on a picture:![][19]
...
There are some crypto currencies with privacy in mind like Monero but even those could be de-anonymized to some extent[^118]'[^119].
...
If you're feeling a bit more adventurous, you could install your own BIOS using Libreboot[^132] or Coreboot[^193] if your laptop supports it.
...
This technology is also widely used in CAPTCHAS[^247] services to verify that you are "human" and can be used to fingerprint a user.
...
Even if you're not looking at the camera, they can still figure out who you are[^154], make out your emotions[^155] and probably guess your political affiliation[^156].
...
-   If you absolutely want system wide plausible deniability[^203] despite the risks[^190].
...
**Always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^194]**.**
...
TAILS[^192] stands for **The Amnesic Incognito Live System.** . It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm>) or System76 laptops (<https://system76.com/>) while using Coreboot[^193] (where Intel IME is disabled from factory).
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. However if you're going for the Qubes Route consider Intel as they do not support AMD with their anti-evil-maid system[^194].

-   Secure Wipe tools from the BIOS (especially useful for SDD drives, see Appendix N: BIOS/UEFI options to wipe disks in various Brands).
...
-   Enable BIOS/UEFI password and use a long passphrase[^195] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^196]. Keep it on if you intend to use Linux/Windows.
...
So what is Secure Boot[^197]? In short, it's a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically when the Operating Systems (or the Bootloader[^198]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^199] (with pictures).
...
This route will make extensive use of Virtual Machines[^200], they'll require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^206]'[^207] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will comes into play.
...
Cold Boot attacks[^210] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you're actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^211], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability. There have been some forensics studies[^212] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^213] and by the maintainer of Veracrypt[^214] so I wouldn't worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^216].

-   Windows Indexing keeping traces of the files present in your User folder by default[^217].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^218]'[^219].
...
-   Gatekeeper[^220] and XProtect keeping track of your download history in a local database
...
Forensics could[^212]'[^215] use all those leaks[^223] to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
Veracrypt[^221] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^222] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^223] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^224] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^225] Operations[^226]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^194] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^233] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
In this route, we will make extensive use of the free Oracle Virtualbox[^234] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the best virtualization software in terms of security and some of the reported issues[^235] have not be completely fixed to this date[^236] and if you're using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM>
...
This route will use Virtualization and Whonix[^237] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
You will need something to store your data (logins/passwords, identities and TOTP[^242] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^243] authentication with the authenticator feature.
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^244], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^246] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^247] authentication with the authenticator feature.
...
Captcha[^248] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^249] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^250]) and Cloudflare (hCaptcha[^251]). hCaptcha is used on 15% of the internet by their own metrics[^252].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^253]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^254].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^255].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^257].
...
Browser and Device[^258] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^259]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^260] on Netflix as they cover this topic much better than anyone else IMHO.
...
This is for instance now the case on YouTube for some European Users[^261].
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^262] and unfortunately sometimes required.
...
Some platforms/apps will require you to take a live picture of yourself either doing something (a wink, holding an arm up ...) or showing a custom piece of information (a hand written text, a passport or ID) within the picture. Sometimes the platform/app will require several pictures to increase their certainty.![][38]

This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^263]. Unfortunately this method is now also sometimes being used on Facebook[^264] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^265])
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^258] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Remember, **always use passphrases instead of passwords and use a different one for each service. Do not make it easy for an adversary to access all your information because you used the same password everywhere**[^194]**.**
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^266].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^267].
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^268] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
If your account gets suspended [^271] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^272] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^273] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They're recommended by privacytools.io[^274]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^275]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the Burner phone and the pre-paid SIM card, you should be safe from tracking.
...
This time you will need to submit an appeal using a form[^276], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You're going to have to find a different way to post there using at least seven proxies[^277] that are not known by 4chan blocking system.
...
-   Use an Android Emulator (Youwave[^278], Bluestacks[^279] or if you have a powerful PC, Android Studio) within the Windows VM and run the App through your multi-layer of Tor/VPN. Drawback is that such emulators are usually quite resource hungry and will slow down your VM and use more battery. Here is also an (outdated) guide on this matter: <https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/>
...
So the fact is that it's very unlikely[^286]'[^287] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it's not completely impossible either[^288]'[^289]'[^290] if they're fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^291] for 11\$ and use it to erase any disk.
...
-   Option B: See Appendix E: Using System Rescue to securely wipe an SSD drive.
...
-   Option C: See Appendix E: Using System Rescue to securely wipe an SSD drive.
...
Use the manufacturer provided tools if possible. Those tools should provide support for safe secure erase or sanitize over USB and are available for most brands: See Appendix K: Manufacturer tools for Wiping HDD and SSD drives:
...
If you don't want to wait for the periodic Trim (set to Weekly by default in Windows 10), you could also force a disk wide Trim using the Windows native Optimize tool (see Appendix I: Windows Cleaning Tools).
...
Please refer to Appendix I: Windows Cleaning Tools and pick a utility before proceeding.
...
-   If your file system is APFS, you don't need to worry about Trim, it apparently happens asynchronously as the OS writes data[^297] according to their own documentation.
...
Unfortunately Apple has removed the secure erase options from the trash bin even for HDD drives[^298]. So you're left with using other tools:
...
Consider also reading this documentation if you're going with Whonix <https://www.whonix.org/wiki/Anti-Forensics_Precautions>
...
The best tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^299] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It's however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^300] and available on TAILS by default[^301].
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^223]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^215] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally I would just pick Normal Deletion + Trim (Trim itself should be enough[^227]). Secure Deletion with Trim[^224] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^302] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^303] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^304] and Secure Communication Obfuscation[^305] through Steganography[^306] and/or Kleptography[^307] and has many names such as Koalang[^308] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...
-   The Windows USN journal which stores plenty of information[^311].

-   The Windows System Resource Usage Monitor (SRUM)[^312].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^313].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^314]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
-   **Use the manufacturer provided tools to securely erase them if possible (see previous Appendix K).**
...
It's possible Windows will detect your external SSD properly and enable Trim by default. Check if Optimize Works using the Windows Native disk utility as explained in the System Disk section.

### ATA/NVMe Operations (Secure Erase/Sanitize):

**Use the manufacturer provided tools to check and perform these operations** ... It's pretty much the only way to be sure it's not only supported but actually works. Some utilities can tell you if it's supported or not like CrystalDiskInfo[^315] but won't actually check if it's working. See Appendix K: Manufacturer tools for Wiping HDD and SSD drives:
...
According to Apple Documentation[^296], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
-   Thanks to Randall Munroe at XKCD
...
[^19]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)>

[^20]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network>
...
[^25]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole>
...
[^31]: Wikipedia, ECH, <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^32]: Mozilla Blog, Encrypted Client Hello: the future of ESNI in Firefox <https://blog.mozilla.org/security/2021/01/07/encrypted-client-hello-the-future-of-esni-in-firefox/>

[^33]: Usenix.org, On the Importance of Encrypted-SNI (ESNI) to Censorship Circumvention <https://www.usenix.org/system/files/foci19-paper_chai_0.pdf>

[^34]: Wikipedia, CDN <https://en.wikipedia.org/wiki/Content_delivery_network>

[^35]: Cloudflare, Good-bye ESNI, hello ECH! <https://blog.cloudflare.com/encrypted-client-hello/>

[^36]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/>

[^37]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/>

[^38]: KUL, Encrypted DNS=⇒Privacy? A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf>

[^39]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries>

[^40]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/>

[^41]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity>

[^42]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity>

[^43]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers>

[^44]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US>

[^45]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law>
...
[^48]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television>

[^49]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html>

[^50]: The Intercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/>

[^51]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher>

[^52]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker>

[^53]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous \'Stingrays\' Become \'Obsolete\' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778>

[^54]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack>

[^55]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^56]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^57]: Purism, Librem 5 <https://shop.puri.sm/shop/librem-5/>

[^58]: Wikipedia, MAC Address <https://en.wikipedia.org/wiki/MAC_address>

[^59]: ResearchGate, Tracking Anonymized Bluetooth Devices <https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf>

[^60]: Apple, Differential Privacy White Paper <https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf>

[^61]: Wikipedia, Differential Privacy <https://en.wikipedia.org/wiki/Differential_privacy>

[^62]: Google Android Help, Android Location Services <https://support.google.com/accounts/answer/3467281?hl=en>

[^63]: Apple Support, Location Services and Privacy <https://support.apple.com/en-us/HT207056>

[^64]: State University of New York, Towards 3D Human Pose Construction Using Wi-Fi <https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf>

[^65]: Using Metadata to find Paul Revere (<https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/>)

[^66]: Wikipedia, Google SensorVault, <https://en.wikipedia.org/wiki/Sensorvault>

[^67]: NRKBeta, My Phone Was Spying on Me, so I Tracked Down the Surveillants <https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/>

[^68]: New York Times <https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html>

[^69]: Sophos, Google data puts innocent man at the scene of a crime <https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/>

[^70]: Wikipedia, Geofence Warrant <https://en.wikipedia.org/wiki/Geo-fence_warrant>

[^71]: Wikipedia, Room 641A <https://en.wikipedia.org/wiki/Room_641A>

[^72]: Wikipedia, Edward Snowden <https://en.wikipedia.org/wiki/Edward_Snowden>

[^73]: Wikipedia, Permanent Record <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)>

[^74]: Wikipedia, XKEYSCORE <https://en.wikipedia.org/wiki/XKeyscore>

[^75]: ElectroSpaces, Danish military intelligence uses XKEYSCORE to tap cables in cooperation with the NSA <https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html>

[^76]: Wikipedia, MUSCULAR <https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)>

[^77]: Wikipedia, PRISM <https://en.wikipedia.org/wiki/PRISM_(surveillance_program)>

[^78]: Justsecurity, General Hayden <https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/>

[^79]: Reuters, Exclusive: Apple dropped plan for encrypting backups after FBI complained -- sources <https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT>

[^80]: ZDnet, I asked Apple for all my data. Here\'s what was sent back <https://www.zdnet.com/article/apple-data-collection-stored-request/>

[^81]: Wired, The Strava Heat Map and the End of Secrets <https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/>

[^82]: Bellingcat, How to Use and Interpret Data from Strava\'s Activity Map <https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/>

[^83]: The Guardian, Fitness tracking app Strava gives away location of secret US army bases <https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases>

[^84]: Telegraph, Running app reveals locations of secret service agents in MI6 and GCHQ <https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/>

[^85]: De Correspondent, Here's how we found the names and addresses of soldiers and secret agents using a simple fitness app <https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27>

[^86]: Washington Post, Alexa has been eavesdropping on you this whole time <https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59>

[^87]: CryptoEngineering, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^88]: Apple Support <https://support.apple.com/en-us/HT210515>

[^89]: XDA, Samsung's Find My Mobile app can locate Galaxy devices even when they're offline <https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/>

[^90]: Apple Support, If your Mac is lost or stolen <https://support.apple.com/en-us/HT204756>

[^91]: Wikipedia, BLE <https://en.wikipedia.org/wiki/Bluetooth_Low_Energy>

[^92]: Cryptography Engineering Blog, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^93]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification>

[^94]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication>

[^95]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/>

[^96]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/>

[^97]: Zoom Support, Adding a watermark <https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark>

[^98]: Zoom Support, Audio Watermark <https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark>

[^99]: CreativeCloud Extension, IMATAG <https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html>

[^100]: NexGuard, <https://dtv.nagra.com/nexguard-forensic-watermarking>

[^101]: Vobile Solutions, <https://www.vobilegroup.com/solutions>

[^102]: Cinavia, <https://www.cinavia.com/languages/english/pages/technology.html>

[^103]: Imatag, <https://www.imatag.com/>

[^104]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^105]: IEEExplore, A JPEG compression resistant steganography scheme for raster graphics images <https://ieeexplore.ieee.org/document/4428921?tp=&arnumber=4428921&url=http:%2F%2Fieeexplore.ieee.org%2Fxpls%2Fabs_all.jsp%3Farnumber%3D4428921>

[^106]: ScienceDirect, Robust audio watermarking using perceptual masking <https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140>

[^107]: IEEExplore, Spread-spectrum watermarking of audio signals <https://ieeexplore.ieee.org/abstract/document/1188746>

[^108]: Google Scholar, source camera identification <https://scholar.google.com/scholar?q=source+camera+identification>

[^109]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html>

[^110]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/>

[^111]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/>

[^112]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/>

[^113]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/>

[^114]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117>

[^115]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524>
...
[^117]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer>

[^118]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y>

[^119]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/>

[^120]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007>

[^121]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)>

[^122]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting>

[^123]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/>

[^124]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach>

[^125]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)>

[^126]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit>

[^127]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine>

[^128]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor>

[^129]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities>

[^130]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor>

[^131]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME>

[^132]: Libreboot, <https://libreboot.org/>

[^133]: Digi.Ninja, Jasager <https://digi.ninja/jasager/>

[^134]: Hak5 Shop, Wi-Fi Pineapple <https://shop.hak5.org/products/wifi-pineapple>

[^135]: Wikipedia, Deautentication Attack <https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack>

[^136]: Wikipedia, Capture Portal <https://en.wikipedia.org/wiki/Captive_portal>

[^137]: Wired, Why the Security of USB Is Fundamentally Broken <https://www.wired.com/2014/07/usb-security/>

[^138]: Wikipedia, Stuxnet <https://en.wikipedia.org/wiki/Stuxnet>

[^139]: Superuser.com, How do I safely investigate a USB stick found in the parking lot at work? <https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work>

[^140]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/>

[^141]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/>

[^142]: ArsTechnica, How the way you type can shatter anonymity---even on Tor <https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/>

[^143]: Paul Moore Blog, Behavioral Profiling: The password you can\'t change. <https://paul.reviews/behavioral-profiling-the-password-you-cant-change/>

[^144]: EFF Panopticlick (<https://panopticlick.eff.org/>)

[^145]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/>
...
[^147]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics>

[^148]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting>

[^149]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/>

[^150]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/>

[^151]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html>
...
[^153]: Brown Institute, Open Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^154]: NewScientist, Facebook can recognize you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^155]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^156]: TechCrunch, Facial recognition reveals political party in troubling new research <https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/>

[^157]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^158]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^159]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^160]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^161]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^162]: Anyvision <https://www.anyvision.co/>

[^163]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^164]: YouTube, The Economist, China: facial recognition and state control \| The Economist <https://www.youtube.com/watch?v=lH2gMNrUuEY>

[^165]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/>

[^166]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/>

[^167]: BBC, WhatsApp photo drug dealer caught by \'groundbreaking\' work <https://www.bbc.com/news/uk-wales-43711477>

[^168]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^169]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^170]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^171]: Wikipedia, Deepfake <https://en.wikipedia.org/wiki/Deepfake>

[^172]: Wikipedia, Deepfake Events <https://en.wikipedia.org/wiki/Deepfake#Example_events>

[^173]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^174]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^175]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^176]: Grayshirt, <https://www.grayshift.com/>

[^177]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf>

[^178]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order>

[^179]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter>

[^180]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html>

[^181]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information>

[^182]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1>

[^183]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^184]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order>

[^185]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary>

[^186]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/>

[^187]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432>

[^188]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/>

[^189]: Ben Nassi, Lamphone, <https://www.nassiben.com/lamphone>

[^190]: Defuse.ca, TrueCrypt\'s Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm>

[^191]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^192]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^193]: Coreboot, <https://www.coreboot.org/>

[^194]: Qubes OS, Anti-Evil Maid, <https://github.com/QubesOS/qubes-antievilmaid>

[^195]: XKCD, Password Strength <https://xkcd.com/936/>

[^196]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported>

[^197]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot>

[^198]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting>

[^199]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^200]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^201]: Sneak.Berlin , Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^202]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^203]: Wikipedia, APNS <https://en.wikipedia.org/wiki/Apple_Push_Notification_service>

[^204]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^205]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^206]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^207]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/>

[^208]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^209]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^210]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^211]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^212]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^213]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^214]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^215]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^216]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^217]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^218]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^219]: BlackBag, Windows 10 Jump List Forensics <https://www.blackbagtech.com/blog/windows-10-jump-list-forensics/>

[^220]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^221]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^222]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^223]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^224]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^225]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^226]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^227]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^228]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^229]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^230]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^231]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^232]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^233]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^234]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^235]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987>

[^236]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox>

[^237]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^238]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^239]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^240]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)>

[^241]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)>

[^242]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^243]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^244]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^245]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>

[^246]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^247]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^248]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^249]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^250]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^251]: hCaptcha <https://www.hcaptcha.com/>

[^252]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^253]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^254]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^255]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^256]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>
...
[^258]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^259]: Developers Google Blog,
...
[^260]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^261]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^262]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^263]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^264]: DigitalInformationWorld, Facebook will now Require you to Create a Video Selfie for Identity Verification! [https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html\#]

[^265]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>

[^266]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^267]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^268]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^269]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^270]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^271]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^272]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^273]: Jumio, ID verification features <https://www.jumio.com/features/>

[^274]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^275]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^276]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^277]: KnowYourMeme, Good Luck, I\'m Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies>

[^278]: Youwave, <https://youwave.com/>

[^279]: Bluestacks, <https://www.bluestacks.com/>

[^280]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module>

[^281]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^282]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM>

[^283]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification>

[^284]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages>

[^285]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection>

[^286]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/>

[^287]: ResearchGate, Live forensics method for acquisition on the Solid State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function>

[^288]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/>

[^289]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/>

[^290]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand>

[^291]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^292]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^293]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli>

[^294]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/>

[^295]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/>

[^296]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^297]: Apple Developer Documentation, <https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html>

[^298]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos>

[^299]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/>

[^300]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata>

[^301]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/>

[^302]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^303]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^304]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^305]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^306]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^307]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^308]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^309]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^310]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^311]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^312]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^313]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^314]: DBAN, <https://dban.org/>

[^315]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/>
  ...
  [Appendix E: Using System Rescue to securely wipe an SSD drive.]: #appendix-e-using-system-rescue-to-securely-wipe-an-ssd-drive.
  ...
  [10]: #linux-3
  [11]: #trim-support-1
  [12]: #atanvme-operations-secure-erasesanitize-1
  [13]: #macos-5
  [14]: #trim-support-2
  [15]: #atanvme-operations-secure-erasesanitize-2
  ...
  [Appendix N: BIOS/UEFI options to wipe disks in various Brands]: #appendix-n-biosuefi-options-to-wipe-disks-in-various-brands
  [Acknowledgements:]: #acknowledgements
  [16]: media/image1.jpeg
  [www.pornhub.com]: http://www.pornhub.com
  [17]: media/image2.jpeg
  [18]: media/image3.jpeg
  [19]: media/image4.jpeg
  [20]: media/image5.jpeg
  [21]: media/image6.jpeg
  [22]: media/image7.jpeg
  [23]: media/image8.jpeg
  [24]: media/image9.jpeg
  [25]: media/image10.jpeg
  [26]: media/image11.jpeg
  [27]: media/image12.jpeg
  [28]: media/image13.jpeg
  [29]: media/image14.jpeg
  [30]: media/image15.jpeg
  [31]: media/image16.jpeg
  [32]: media/image17.jpeg
  [33]: media/image18.jpeg
  ...
  [34]: media/image19.jpeg
  [35]: media/image20.jpeg
  [36]: media/image21.jpeg
  [37]: media/image22.jpeg
  [38]: media/image23.jpeg
  [39]: media/image24.jpeg
  [https://ProtonMail.com/terms-and-conditions]: https://protonmail.com/terms-and-conditions
  [40]: media/image25.jpeg
  [41]: media/image26.jpeg
  [42]: media/image27.jpeg
  [43]: media/image28.jpeg
  [44]: media/image29.jpeg
  [45]: media/image30.jpeg
  [46]: media/image31.jpeg
  [47]: media/image32.jpeg
