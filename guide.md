...
Version 0.5.6 (draft), January 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
Here is an example of EXIF data that could be on a picture:![][12]
...
There are some crypto currencies with privacy in mind like Monero but even those could be de-anonymized to some extent[^117][^118].
...
So the fact is that it's very unlikely[^284]'[^285] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it's not completely impossible either[^286]'[^287]'[^288] if they're fast enough and have access to extensive equipment, skills and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^289] for 11\$ and use it to erase any disk.
...
-   Trim and then fill up the entire free space of the disk using an utility such as BleachBit or PrivaZer.
...
-   Decrypt and re-encrypt (using LUKS for instance) the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space.
...
If Trim is not supported or you want to be sure, you might have to ensure secure data deletion by filling up the entire free space of the disk using an utility such as:

-   Decrypt and re-encrypt (using LUKS or Veracrypt for instance) the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space.
...
Unfortunately Apple has removed the secure erase options from the trash bin even for HDD drives[^295]. So you're left with using other tools:
...
If Trim is supported and enabled by MacOS for your external SSD drive (unlikely). There should be no issue in securely deleting data.

If Trim is not supported (likely), you might have to ensure secure data deletion by:
...
The best tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^296] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It's however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^297] and available on TAILS by default[^298].
...
##### Quarantine Database (used by Gatekeepr and XProtect):
...
DuckDuckGo does not store cached version of pages[^299] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^300] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^301] and Secure Communication Obfuscation[^302] through Steganography[^303] and/or Kleptography[^304] and has many names such as Koalang[^305] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...
As written earlier in this guide and as noted by Privacytools.io[^306], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   System Rescue:
    ...
    -   Disable Secure Boot in your BIOS/UEFI settings and change the boot order to the USB disk (System Rescue bootloader is not signed and won't boot with secureboot enabled).
...
-   The Windows USN journal which stores plenty of information[^308].

-   The Windows System Resource Usage Monitor (SRUM)[^309].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^310].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^311]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
**WARNING: using ATA/NVMe Secure Erase through USB can be dangerous. Please read the disclaimers here <https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase> and here <https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing>**
...
[^289]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^290]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^291]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli>

[^292]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/>

[^293]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/>

[^294]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^295]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos>

[^296]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/>

[^297]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata>

[^298]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/>

[^299]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^300]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^301]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^302]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^303]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^304]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^305]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^306]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^307]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^308]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^309]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^310]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^311]: DBAN, <https://dban.org/>
  ...
  [10]: media\image2.jpeg
  [11]: media\image3.jpeg
  [12]: media\image4.jpeg
  [13]: media\image5.jpeg
  [14]: media\image6.jpeg
  [15]: media\image7.jpeg
  [16]: media\image8.jpeg
  [17]: media\image9.jpeg
  [18]: media\image10.jpeg
  [19]: media\image11.jpeg
  [20]: media\image12.jpeg
  [21]: media\image13.jpeg
  [22]: media\image14.jpeg
  [23]: media\image15.jpeg
  [24]: media\image16.jpeg
  [25]: media\image17.jpeg
  ...
  [26]: media\image18.jpeg
  [27]: media\image19.jpeg
  [28]: media\image20.jpeg
  [29]: media\image21.jpeg
  [30]: media\image22.jpeg
  [31]: media\image23.jpeg
  [https://ProtonMail.com/terms-and-conditions]: https://protonmail.com/terms-and-conditions
  [32]: media\image24.jpeg
  [33]: media\image25.jpeg
  [34]: media\image26.jpeg
  [35]: media\image27.jpeg
  [36]: media\image28.jpeg
  [37]: media\image29.jpeg
  [38]: media\image30.jpeg
  [39]: media\image31.jpeg
